...
> **推荐：镜像拉取模式**。GitHub Actions 已自动构建镜像并推送到 Docker Hub（国内国外都能拉，不绑云厂商），服务器端 `docker compose pull && up -d` 即可，**不再需要在 2C2G 小机上跑 pnpm install + vite build**。完整配置见 [deploy-docker.md](./deploy-docker.md)；仅腾讯云国内轻量想走内网的用户额外可启用 TCR 详见 [deploy-tcr.md](./deploy-tcr.md)。
>
...
CI 自动构建镜像并推送到 Docker Hub（`docker.io/chiyoumask/quantforge`）。宝塔默认已配置 Docker 加速器（见 §1），可直接拉。

```bash
cd /www/wwwroot/quantforge
git pull                                  # 拉最新 docker-compose.yml
docker compose pull                       # 从 Docker Hub 拉镜像（约 1-3 分钟）
docker compose up -d                      # 启动 (不再 --build)
docker compose logs -f app                # 看到 "ready; N capabilities active" 即成功
```

> 公开镜像**无需 docker login**。若反复拉超时，参考 [deploy-docker.md §1.3](./deploy-docker.md) 换镜像加速器或 `.env` 覆盖 `APP_IMAGE=ghcr.io/chiyoumask/quantforge:latest`。
> `data/` 卷不动，数据不丢；之后每次只拉增量层，通常 < 10 秒。
...
docker compose pull                # 拉最新镜像（Docker Hub）
...
> 腾讯云国内轻量走 TCR 内网的用户可改用：`APP_IMAGE=ccr.ccs.tencentyun.com/<namespace>/quantforge:sha-<commit>`，详见 [deploy-tcr.md](./deploy-tcr.md)。
...
| `docker compose pull` 慢 / 超时 | Docker Hub 国内公网慢 | 参 [deploy-docker.md §1.3](./deploy-docker.md)：配 §1 镜像加速器；或 `.env` 覆盖 `APP_IMAGE=ghcr.io/chiyoumask/quantforge:latest` |
| `docker compose pull` 报 unauthorized | Docker Hub 偶发匿名限流 | `docker login docker.io`（无账号也能匿名拉，登录去匿名限流） |
| 腾讯云轻量想走内网最快 | 公网拉 Docker Hub 仍慢 | 仓库 owner 配 TCR secrets 并 `.env` 改 `APP_IMAGE=ccr.ccs.tencentyun.com/<ns>/quantforge:latest`，详见 [deploy-tcr.md](./deploy-tcr.md) |
